...
class test_tokenizer(unittest.TestCase):
    ...
    def test_readtokenword(self):
        s = 'a\\"'
        self.assertTokens(s, [t(tt.WORD, 'a\\"', [0, len(s)], set([flags.word.QUOTED]))])
    ...
    def test_parameter_expansion(self):
        ...
        s = "a $$"
        self.assertTokens(s, [t(tt.WORD, "a", [0, 1]), t(tt.WORD, "$$", [2, 4], hasdollarset)])

    def test_comsub(self):
        s = "a $(b)"
        self.assertTokens(s, [t(tt.WORD, "a", [0, 1]), t(tt.WORD, "$(b)", [2, 6], hasdollarset)])
        ...
        s = '"$(a)"'
        self.assertTokens(
            s, [t(tt.WORD, '"$(a)"', [0, 6], set([flags.word.HASDOLLAR, flags.word.QUOTED]))]
        )

        s = "a $(! b)"
        self.assertTokens(s, [t(tt.WORD, "a", [0, 1]), t(tt.WORD, "$(! b)", [2, 8], hasdollarset)])
        ...
        self.assertTokens(
            s, [t(tt.WORD, "$(case a in (b) c ;; (d) e ;; esac)", [0, len(s)], hasdollarset)]
        )
        ...
        s = "$(a <<EOF\nb\nEOF)"
        self.assertTokens(s, [t(tt.WORD, "$(a <<EOF\nb\nEOF)", [0, len(s)], hasdollarset)])
        ...
        s = "$(a # comment\n)"
        self.assertTokens(s, [t(tt.WORD, "$(a # comment\n)", [0, len(s)], hasdollarset)])

    def test_parsematchedpair(self):
        s = '"`foo`"'
        self.assertTokens(s, [t(tt.WORD, '"`foo`"', [0, len(s)], set([flags.word.QUOTED]))])

        s = '"${a}"'
        self.assertTokens(
            s, [t(tt.WORD, '"${a}"', [0, len(s)], set([flags.word.HASDOLLAR, flags.word.QUOTED]))]
        )
    ...
    def test_heredoc(self):
        s = "a <<EOF"
        self.assertTokens(
            s, [t(tt.WORD, "a", [0, 1]), t(tt.LESS_LESS, "<<", [2, 4]), t(tt.WORD, "EOF", [4, 7])]
        )
    ...
    def test_tokenize(self):
        ...
        s = "wx    y =z "
        self.assertTokens(
            s, [t(tt.WORD, "wx", [0, 2]), t(tt.WORD, "y", [6, 7]), t(tt.WORD, "=z", [8, 10])]
        )
