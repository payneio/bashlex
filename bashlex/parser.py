...
def _expandword(parser, tokenword):
    if parser._expansionlimit == -1:
        ...
        node = ast.node(
            kind="word", word=tokenword, pos=(tokenword.lexpos, tokenword.endlexpos), parts=[]
        )
        return node
    else:
        ...
        node = ast.node(
            kind="word", word=expandedword, pos=(tokenword.lexpos, tokenword.endlexpos), parts=parts
        )
        return node
...
def parsesingle(s, strictmode=True, expansionlimit=None, convertpos=False, proceedonerror=False):
    ...
    p = _parser(
        s, strictmode=strictmode, expansionlimit=expansionlimit, proceedonerror=proceedonerror
    )
    tree = p.parse()
    if convertpos:
...
class _parser(object):
    ...
    def __init__(
        self, s, strictmode=True, expansionlimit=None, tokenizerargs=None, proceedonerror=None
    ):
