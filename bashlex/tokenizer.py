...
class tokenizer(object):
    ...
    def _readtokenword(self, c):
        ...
        d["dollar_present"] = d["quoted"] = d["pass_next_character"] = d["compound_assignment"] = (
            False
        )
        ...
        if (
            ...
            and (
                c in "<>"
                or self._last_read_token.ttype in (tokentype.LESS_AND, tokentype.GREATER_AND)
            )
    ...
    def _parse_comsub(
        self, doublequotes, open, close, parsingcommand=False, dquote=False, firstclose=False
    ):
        ...
        checkcase = bool(
            parsingcommand and (doublequotes is None or doublequotes not in "'\"") and not dquote
        )
    ...
    def _command_token_position(self, token):
        return (
            ...
            or (
                token.ttype
                not in (tokentype.SEMI_SEMI, tokentype.SEMI_AND, tokentype.SEMI_SEMI_AND)
